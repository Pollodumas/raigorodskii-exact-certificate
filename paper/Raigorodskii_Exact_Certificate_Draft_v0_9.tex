...
\date{Draft v0.9, 12 August 2026}
...
Part~(v) follows from the elementary positivity certificate in Proposition~\ref{prop:two-roots} below. The verification programs also compute an independent exact Sturm sequence with the same root count.
\end{proof}


\begin{proposition}[Elementary certificate for the two real roots]\label{prop:two-roots}
The polynomial \(Q\) has exactly one root in
\( (-467/1000,-466/1000) \), exactly one root in \( (6/5,13/10) \), and no other real roots.
\end{proposition}

\begin{proof}
For \(0\le t\le1\), write
\[
B_{i,n}(t)=\binom ni t^i(1-t)^{n-i}.
\]
Every \(B_{i,n}\) is nonnegative on \([0,1]\), and
\(\sum_{i=0}^nB_{i,n}(t)=1\).

First, for \(y\ge0\), exact expansion gives
\[
\begin{aligned}
Q\!\left(-\frac{467}{1000}-y\right)={}&144y^6+\frac{116436}{125}y^5
+\frac{29399403}{12500}y^4+\frac{8985296567}{3125000}y^3\\
&+\frac{21570510479867}{12500000000}y^2
+\frac{13886577206994389}{31250000000000}y
+\frac{21899765866381721}{62500000000000000}.
\end{aligned}
\]
All coefficients are positive, so \(Q(x)>0\) for \(x\le-467/1000\).

Next set
\[
x=-\frac{466}{1000}+\frac{833}{500}t,
\]
which maps \([0,1]\) onto \([-466/1000,6/5]\). Exact conversion to the degree-eight Bernstein basis gives
\[
-Q(x)=\sum_{i=0}^8 b_iB_{i,8}(t),
\]
where
\[
\begin{aligned}
(b_0,\ldots,b_8)=\bigg(&
\frac{90089305534379}{976562500000000},
\frac{359044792692858729}{3906250000000000},
\frac{52870090497122397}{3906250000000000},\\
&\frac{124620702915641}{78125000000000},
\frac{13443213954293}{390625000000},
\frac{282934224509}{6250000000},\\
&\frac{3214343901}{125000000},
\frac{118373431}{6250000},
\frac{49351}{15625}\bigg).
\end{aligned}
\]
Every \(b_i\) is positive. Hence \(Q(x)<0\) throughout \([-466/1000,6/5]\).

On the remaining interval put
\[
x=-\frac{467}{1000}+\frac{t}{1000}.
\]
A degree-five Bernstein expansion gives
\[
-Q'(x)=\sum_{i=0}^5 c_iB_{i,5}(t),
\]
with
\[
\begin{aligned}
(c_0,\ldots,c_5)=\bigg(&
\frac{13886577206994389}{31250000000000},
\frac{6932503348257261}{15625000000000},
\frac{3460865785481089}{7812500000000},\\
&\frac{1727743314228061}{3906250000000},
\frac{862528548927789}{1953125000000},
\frac{430593559988861}{976562500000}\bigg).
\end{aligned}
\]
Thus \(-Q'(x)>0\), so \(Q\) is strictly decreasing on
\([-467/1000,-466/1000]\). Since
\[
Q\!\left(-\frac{467}{1000}\right)
=\frac{21899765866381721}{62500000000000000}>0,
\qquad
Q\!\left(-\frac{466}{1000}\right)
=-\frac{90089305534379}{976562500000000}<0,
\]
continuity gives exactly one root in that interval. Part~(iii) gives exactly one root in \((6/5,13/10)\) and none on \([13/10,\infty)\). Together with the sign certificates on
\(( -\infty,-467/1000]\) and \([-466/1000,6/5]\), these intervals cover \(\mathbb R\) and prove the claim.
\end{proof}
...
\begin{remark}[Minimal certificate primes]
Among primes not dividing \(\operatorname{disc}(R)\), an exact finite scan shows that \(61\) is the least prime for which \(R\bmod p\) has factor degrees \([1,5]\), and \(107\) is the least prime for which it has factor degrees \([1,1,1,1,2]\). This minimality is not needed for the proof, but explains the choice of the two Dedekind primes.
\end{remark}
...
The first uses SymPy 1.14.0 as an exact-arithmetic engine. It checks the derivative identity and its positivity decomposition, the resultant, a lexicographic Gr\"obner elimination, the minimal polynomial of the exact optimizer value, the primitive content, the Rabin certificate over \(\mathbb F_5\), both exact real-root certificates, the endpoint brackets, and the modular factorizations used in Corollary~\ref{cor:galois} \cite{SymPy2017}.

The second uses only Python's standard library. It derives the finite-field reduction from \(Q\), implements polynomial arithmetic over finite fields, computes the full Sylvester resultant symbolically by fraction-free Bareiss elimination over \(\mathbb Z[x]\), constructs rational Sturm sequences, verifies independently the Bernstein-basis certificate of Proposition~\ref{prop:two-roots}, derives \(R\) and its discriminant from \(Q\), and checks the endpoint, integral-scaling, Dedekind, and Galois-factorization certificates. Both programs also verify that \(61\) and \(107\) are the least unramified primes with the required factor patterns.

All load-bearing Python checks use explicit exceptions rather than the \texttt{assert} statement, so they remain active under \texttt{python -O}. A directed mutation harness corrupts coefficients, enclosures, Bernstein data, and cycle-prime claims and requires every corruption to be detected in normal execution, with representative corruptions repeated under optimized execution.

A Lean 4 companion, pinned to Lean and Mathlib version 4.32.1, kernel-checks the analytic and real-algebraic core and the integrality of \(6\gR\). It contains no placeholder proofs or added axioms. The continuous-integration workflow scans all Lean source tokens for placeholders and records \texttt{\#print axioms} output for the principal theorems. Irreducibility, the global root count, minimality of the scaling factor, and the Galois argument are not claimed as fully formalized in Lean.

No floating-point computation is used in any logical step.
...
To the best of our knowledge, this explicit minimal polynomial and exact root isolation have not previously appeared in the literature. Searches included the exact decimal expansion, both coefficient tuples, OEIS, accessible primary texts and bibliographic records for the cited lower-bound papers, and the public Erd\H{o}s-problems record. No prior appearance was found. The search was not comprehensive: in particular, MathSciNet, zbMATH, some paywalled full texts, and Russian-language literature beyond accessible records were not exhaustively searched. No unconditional claim of bibliographic priority is made pending broader specialist review.
...
\newblock \href{https://www.erdosproblems.com/704}{erdosproblems.com/704}, accessed 12 August 2026.
